...
from sqlalchemy.orm import sessionmaker, scoped_session, relationship, joinedload # noqa
...
class User(Base):
    ...
    is_staff = Column(Boolean, nullable=False, default=False, serve_default='0') # noqa
    created_at = Column(DateTime, default=datetime.utcnow, server_default=func.now()) # noqa

    posts = relationship('Post', back_populates='user')

    def __str__(self):
        return f'{self.__class__.__name__}(id={self.id}, username={self.username!r}, is_staff={self.is_staff}, created_at={self.created_at})' # noqa
...
class Post(Base):
    ...
    def __str__(self):
        return f"{self.__class__.__name__}(id={self.id}, title={self.title!r}, user={self.user})" # noqa
